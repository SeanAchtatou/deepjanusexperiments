...
for i in os.listdir("models"):
    if (("low" not in i) and (".h5" in i) and ("Advanced" not in i) and ("Fine" not in i)):
...
for i in classifiers:
    ...
    attack = ProjectedGradientDescent(estimator=i, eps=0.3)
    ...
    for j in final_examples:
        imgs.append(j)
        cv2.imwrite(os.path.join("toolbox_runs"+"/"+path_runs,f"{count}.jpg"),255*j)
        count += 1
